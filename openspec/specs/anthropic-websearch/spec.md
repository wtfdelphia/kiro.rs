# Capability: anthropic-websearch

## Purpose

Serve Anthropic-side web search tool requests in the response shape the client asked for: SSE when `stream` is true, a plain `message` JSON object otherwise. Both modes are built from one shared block constructor so their content blocks and usage accounting stay identical.
