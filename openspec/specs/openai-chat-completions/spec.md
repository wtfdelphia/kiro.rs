...
Serve `POST /v1/chat/completions` as an OpenAI-compatible client-facing endpoint. Requests are mapped into the internal Anthropic-shaped structure and pushed through the existing conversion pipeline, so upstream-side constraints apply identically to both protocols. Responses use OpenAI shapes and error dialect, echo the client's model string, and restore original tool names.
