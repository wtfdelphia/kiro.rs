...
Expose Admin UI entry points so operators can refresh and inspect the model catalog and run credential generation smoke tests without using raw HTTP clients. Consumes existing Admin model-catalog and credential-model-test APIs. Also requires that known-bad fixed profileArn placeholders must not permanently block those entry points for otherwise healthy credentials.
...
- MUST NOT require new backend endpoints solely for UI exposure of model ops
