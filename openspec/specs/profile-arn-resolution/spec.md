...
Resolve and cache Kiro `profileArn` before upstream generateAssistantResponse / MCP / usage-limits requests, so credentials without a pre-stored ARN (especially KAM-imported IdC/BuilderId) can chat successfully. Aligns with Kiro IDE / Kiro-Go resolution semantics.
