...
| Scenarios | PASS（本地）| 7 个 Requirement 的场景由 `scripts/tests/` 23 个用例与本地命令覆盖；镜像 label 场景仅有静态断言，见剩余风险 |
| Project Rules | PASS | OpenSpec 流程完整；`cargo check --release --all-targets` 告警 0，相对基线零新增；无真实凭据、token、`.codegraph/` 进入候选提交 |
| Verification | WARN | 本地命令全部真实运行并记录；Docker inspect 与真实 CI run 明确 SKIPPED 并写入剩余风险 |
...
| 运行时与容器元数据可观测 | `cargo run --quiet -- --version` → `kiro-rs 2026.8.10`；`test_startup_reports_version_before_config_error` 断言首条 info 为 `kiro-rs v...`、早于配置失败日志；`Dockerfile` `ARG VERSION` + OCI label；workflow 传 `build-args` |
...
- 本机无 Docker 与 actionlint：任务 5.3 的镜像 inspect 与 workflow lint 为 SKIPPED，OCI label 值一致性仅有静态断言。
