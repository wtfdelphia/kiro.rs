...
当前已完成到 change 4（`desktop-credentials-view`）加审核修复
（`desktop-review-followups`）：change 2 骨架、change 3 SQLite 存储、
change 4 凭据视图（列表/筛选/启停/删除/测试/余额 + 添加/批量导入/
KAM 导入对话框 + Builder ID / IAM SSO 在线登录）。
