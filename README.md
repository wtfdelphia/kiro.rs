...
<tr>
<td>
<b>上游项目鸣谢</b>：<a href="https://co.yes.vg/register?ref=hank9999">YesCode</a> 曾为上游项目提供 AI API 额度赞助。该链接保留自上游 README，若需体验请自行判断适用性 → <a href="https://co.yes.vg/register?ref=hank9999">立即访问</a>
</td>
</tr>
</table>
...
**Write Failed/会话卡死**: 如果遇到持续的 Write File / Write Failed 并导致会话不可用，可参考上游 Issue [#22](https://github.com/hank9999/kiro.rs/issues/22) 和 [#49](https://github.com/hank9999/kiro.rs/issues/49) 的说明与临时解决方案（通常与输出过长被截断有关，可尝试调低输出相关 token 上限）
...
>
> - 正式版：[https://github.com/wtfdelphia/kiro.rs/releases/latest](https://github.com/wtfdelphia/kiro.rs/releases/latest)
> - 开发滚动包：[https://github.com/wtfdelphia/kiro.rs/releases/tag/dev-latest](https://github.com/wtfdelphia/kiro.rs/releases/tag/dev-latest)
...
| GET/PUT | `/api/admin/settings/websocket`                  | WebSocket ingress 设置（enabled/mode/限额/超时；含活跃连接数；热更新 + 落盘） |
...
| 类型           | 说明                        | 链接                                                                                                                          |
| -------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Releases 列表  | 所有正式版 / prerelease     | [https://github.com/wtfdelphia/kiro.rs/releases](https://github.com/wtfdelphia/kiro.rs/releases)                               |
| 最新正式版     | 最近一次`v*` 自动发布     | [https://github.com/wtfdelphia/kiro.rs/releases/latest](https://github.com/wtfdelphia/kiro.rs/releases/latest)                 |
| 开发滚动包     | `dev` 分支滚动 prerelease | [https://github.com/wtfdelphia/kiro.rs/releases/tag/dev-latest](https://github.com/wtfdelphia/kiro.rs/releases/tag/dev-latest) |
| 指定正式版示例 | 如`v2026.7.27`            | [https://github.com/wtfdelphia/kiro.rs/releases/tag/v2026.7.27](https://github.com/wtfdelphia/kiro.rs/releases/tag/v2026.7.27) |
...
- `Cargo.toml` 的 `[package].version` 是源码版本声明，格式为 `YYYY.MM.MICRO`，例如 `2026.8.11`。
- **第三段是当月发布序号，不是日期。** `2026.8.11` 表示「2026 年 8 月第 11 次发布」。同一年月内可以发布多个正式版本，序号递增即可，同一天也能发多次。
- 正式发布使用与 Cargo 版本一致的附注 tag：`vYYYY.MM.MICRO`。月份和序号不补零，不使用 `-1` 等修订后缀。
- 正式 tag 必须指向已进入 `main` 的发版提交。版本门禁会校验 tag 格式、月份范围、附注类型、tag 指向、Cargo 版本和 `main` 可达性。
- `admin-ui/package.json` 的版本不属于发布身份。Admin UI 随 Rust 二进制内嵌发布，不跟随主项目 CalVer。

发布清单（以 `2026.8.11` 为例）：

```bash
# 1. 查出本月已用的最大序号，下一个正式版在其上加一
git tag -l 'v2026.8.*' | sort -V | tail -1

# 2. 更新 Cargo.toml 的 package.version，并刷新/验证锁文件
cargo check --release --all-targets

# 3. 提交并通过 PR/普通合并进入 main，确认提交可达
git merge-base --is-ancestor <release-commit> origin/main

# 4. 在该提交创建附注 tag，再推送 tag
git tag -a v2026.8.11 -m "Release v2026.8.11" <release-commit>
...
- 二进制：GitHub Releases
- 容器镜像：GHCR 包页 [https://github.com/wtfdelphia/kiro.rs/pkgs/container/kiro-rs](https://github.com/wtfdelphia/kiro.rs/pkgs/container/kiro-rs)
...
- 包页面：[https://github.com/wtfdelphia/kiro.rs/pkgs/container/kiro-rs](https://github.com/wtfdelphia/kiro.rs/pkgs/container/kiro-rs)
...
人工触发 Docker workflow 时，`publish=false`（默认）只执行 dry run，`version` 输入仅作为本次非正式构建标签，不登录或推送 GHCR。设置 `publish=true` 时，workflow 会忽略自由 `version` 输入，并要求所选提交上恰好存在一个合法的附注 `vYYYY.MM.MICRO` tag；该 tag 通过正式版本门禁后才允许发布。
...
| `/v1/responses`        | GET  | OpenAI Responses WebSocket ingress（`Upgrade: websocket`；http_bridge：单连接多 turn，每 turn 桥接一次上游 HTTP/SSE；非 upgrade 请求返回 426） |
...
`GET /v1/responses`（WebSocket）额外注意：

- 事件 JSON 与 `POST /v1/responses` 的 SSE `data:` 行同形（无 keepalive 对应物）
- 单连接多 turn：`response.create` 每帧一轮；`response.cancel` 取消当前 turn；
  `session.update` 记录会话级 `model` 覆盖（后续省略 `model` 的 create 使用它）
- 首帧必须携带非空 `model`；`type` 缺省按 `response.create` 处理
- 运行时开关与限额在 Admin `GET/PUT /api/admin/settings/websocket` 热加载：
  `enabled=false` 只拦新连接、不断存量会话；`mode=passthrough` 为预留项（当前握手前返回 501）
  （Admin UI 设置面板的 WebSocket 分区提供同样的可视化开关与调参）
...
| `type: "namespace"`（工具分组）                      | 内层工具展平为`<namespace>__<name>` 的独立工具，内层 `custom` 同样按展平名做 freeform 降级；响应侧还原为 `(namespace, 原名)`，freeform 调用回 `custom_tool_call` |
...
- [openspec/changes/<change-name></change>/](openspec/changes/)：单次变更 proposal / design / tasks / specs / evidence
